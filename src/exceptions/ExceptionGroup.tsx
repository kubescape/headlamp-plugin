...
import { getItemFromSessionStorage,KubescapeSettings } from '../common/webStorage';
...
export function ExceptionGroupNew() {
  ...
  if (!kubeScapeNamespace) return <div>Loading...</div>;

  return (
    <ExceptionGroup
      group={{ name: '', exceptionPolicies: [] }}
...
export function ExceptionGroupEdit() {
  ...
  if (configMap) {
    const group = {
      ...
      exceptionPolicies: JSON.parse(configMap.jsonData.data.exceptionPolicies ?? '[]'),
