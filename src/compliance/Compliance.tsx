...
async function fetchCustomFrameworks(
  ...
  response?.items?.forEach((configMap: any) => {
    let controlsIDs: string[] = [];
    try {
      controlsIDs = JSON.parse(configMap.data.controlsIDs) ?? [];
    } catch {
      console.error('Failed to parse controlsIDs for framework', configMap.metadata?.name);
    }
